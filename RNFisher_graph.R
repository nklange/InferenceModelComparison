...
source("CompareModels_FisherRN.R") # for quantitative fit plots
...
Quant <- plot_grid(ModelCompTotal,ModelCompSs,labels=c("B","C"),nrow=1,
...
AggPrediction <- AggPrediction %>%
  filter(model %in% c("MK_RNminus","J_RNminus",
                      "MK_RNplus","J_RNplus")) %>%
...
aggerrordistribution <- ggplot(data = experimentfile %>% filter(exp==experiment),
  ...
  scale_x_continuous(limits=c(-pi,pi),name = "Error (radian)",breaks = c(-pi/2,0,pi/2),
                     labels = c(expression(-frac(pi,2)),
                                0,
                                expression(frac(pi,2)))) +
  coord_cartesian(xlim = c(-pi/2,pi/2))+
  geom_line(data = AggPrediction,
            aes(x = data,y=prediction,linetype=factor(model),color=factor(model),group=factor(model)),
            size = 0.75) +
...
experimentfile <- bind_rows(data_ol17[data_ol17$exp == "ol17_e1",],data_vdb14,data_pratte17)  %>%
  rename(setsize = "set_size") %>% filter(model %in% models)
...
plot_grid(datastabpoints,Quant,aggerrordistribution,SumStat,labels = c("A",NA,"D",NA),ncol=1,
          rel_heights = c(0.6,0.9,0.8,0.7))

#ggsave("RQ1.png", units="cm", width=35, height=40, dpi=600)
...
models <- c("MK_RNminus", "MK_RNplus",
    ...
    LegendInfo <- c(0.25,0.3)
    StripLabel <- element_text(size=9)
    Ticks <- element_line()
  ...
  FittedFull <- BasisFit %>%
    ...
    mutate(FactorJ = ifelse(grepl("J",model),"Present","Absent")) %>%
    mutate(type = case_when(model == "MK_RNminus" ~ 1,
                            model %in% c("J_RNplus") ~ 3,
                            TRUE ~ 2)) %>%
    #filter(model %in% CapUmodels) %>%
    group_by(exp,id) %>% arrange(exp,id,type,FactorRN,FactorJ)
  ...
  LFLR <- KIDKOD %>% mutate(expon = exp(-best)) %>%
    summarize(RNFactor = log(mean(expon[3:4])/mean(expon[1:2])),
              JFactor = log(mean(expon[c(2,4)])/mean(expon[c(1,3)]))) %>%
  ...
  FIM <- bind_rows(KID,KOD,LFLR) %>% group_by(exp,measure,FIM) %>%
    ...
    mutate(measure = factor(measure,levels = c("RNFactor",
                                               "JFactor"),
                            labels = c("Response Noise","Fisher Information"))) %>%
    #mutate(Lim = limLabel) %>%
 ...
 expgroup<-ggplot(data=FIMind, aes(x = exp,y=meanFIM,fill=measure)) +
   ...
   tot <- ggplot(FIMind, aes(x = 1,y=meanFIM,fill=measure)) +
     geom_hline(yintercept = 0) +
     geom_hline(yintercept = 9.2,linetype="dashed",color="grey") +
     geom_hline(yintercept = 6.8,linetype="dashed",color="grey") +
     geom_hline(yintercept = 4.6,linetype="dashed",color="grey") +
...
FIM <-plot_grid(RNFisherFIM(FittedFull),RNFisherFIM(LOTraining),nrow=1,rel_widths=c(1,1),labels=c("A","B"),scale=.95)

Exps <- ggdraw() + draw_image("StimuliExp.png")

plot_grid(FIM,Exps,nrow=2,rel_heights = c(2.5,1),labels = c(NA,"C"))

#ggsave("FIM_FisherRN.png", units="cm", width=35, height=20, dpi=600)
