...
models <- c("MK_RNminus","MK_P_RNminus","MK_U_RNminus","MK_FM_RNminus",
...
oldmodellabels <- c("MK_RNminus","MK_FM_RNminus","MK_P_RNminus","MK_U_RNminus",
                    "MK_RNplus","MK_FM_RNplus","MK_P_RNplus","MK_U_RNplus")

newmodellabels <- c("VP(K)A-","VP(K)F-","VP(K)P-","VP(K)U-",
                    "VP(K)A+","VP(K)F+","VP(K)P+","VP(K)U+")

manuscriptmodelorder <- c("VP(K)U-","VP(K)A-","VP(K)P-","VP(K)F+","VP(K)F-","VP(K)A+","VP(K)P+","VP(K)U+")
...
ModelCompTotal <- ggplot(Total %>% filter(!method == "BIC"),
  ...
  geom_rect(aes(xmin=0, xmax=8.5, ymin=0, ymax=1), fill = "grey") +
...
#
#   geom_text(data = Nleft,aes(y = 9.2, x = exp,
...
#
# ggsave("SuppRQ2.png", units="cm", width=30, height=40, dpi=600)
